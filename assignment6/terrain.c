...
void loadMap(Map* maps[MAX_MAPS][MAX_MAPS], char map[HEIGHT][WIDTH], int x, int y) {
    ...
    if (maps[x][y]) {
        ...
        // Place adversaries on the new map, checking if they have not been defeated
        rivalDefeated = 0;
        hikerDefeated = 0;
        pacerDefeated = 0;
        explorerDefeated = 0;
    ...
    // Find and set the position for special NPCs like Pacer if they haven't been set already
    bool foundPacer = false;
    for (int row = 0; row < HEIGHT && !foundPacer; row++) {
        for (int col = 0; col < WIDTH && !foundPacer; col++) {
            if (map[row][col] == PACER) {
                pacerX = col; // Set specific Pacer X for this map
                pacerY = row; // Set specific Pacer Y for this map
                foundPacer = true;
                pacerDirection = (rand() % 2) * 2 - 1; // Randomly left (-1) or right (1)
                break;
            }
        }
    }
    if (!foundPacer) {
        pacerX = -1; // Indicate no Pacer on this map
        pacerY = -1;
    }
    if(!wandererfound){
        WandererX = -1;
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        switch(ch) {
            ...
            scanw("%d %d", &newPlayerX, &newPlayerY); // ncurses way to get formatted input
            ...
            if (newPlayerX >= -200 && newPlayerX < 200 && newPlayerY >= -200 && newPlayerY < 200) {
                ...
                currentX = newPlayerX + MAX_MAPS / 2; // Adjusting as per your world coordinates
                currentY = newPlayerY + MAX_MAPS / 2;
                loadMap(maps, map, currentX, currentY); // Load or generate the new map

                // Search for existing player position on the loaded map
                bool foundPlayer = false;
                for (int y = 0; y < HEIGHT && !foundPlayer; y++) {
                    for (int x = 0; x < WIDTH && !foundPlayer; x++) {
                        if (map[y][x] == PLAYER) {
            ...
            clear(); // Clear the window before reprinting the map
            break;
        ...
        if (ch == 't') {
            ...
            if ((newX == 0 || newX == WIDTH - 1 || newY == 0 || newY == HEIGHT - 1) && newPosType == PATH) {
                    int newMapX = currentX;
                    int newMapY = currentY;
                    if (newX == 0) newMapX--;
                    else if (newX == WIDTH - 1) newMapX++;
                    if (newY == 0) newMapY--;
                    else if (newY == HEIGHT - 1) newMapY++;
                    ...
                    // Load the new map, or generate if it doesn't exist
                    loadMap(maps, map, currentX, currentY);
                    
                    if (newX == 0) playerX = WIDTH - 2;
                    else if (newX == WIDTH - 1) playerX = 1;
                    if (newY == 0) playerY = HEIGHT - 2;
                    else if (newY == HEIGHT - 1) playerY = 1;

                    map[playerY][playerX] = PLAYER; // Place the player
                    prevPlayerCell = PATH; // Assume the player comes into a path

                    // Refresh the NPC paths based on the new player location
                    dijkstra(map, &distMapHiker, playerX, playerY, 0);
                    dijkstra(map, &distMapRival, playerX, playerY, 1);
                }
