...
void printMap(char map[HEIGHT][WIDTH]) {
    ...
    //Gets the cordinates displayed at the bottom
    printf("(%d, ", currentX -200);
    printf("%d) ", currentY -200);
...
int main() {
    ...
    // Initialize and save the initial map
    initializeMap(map);
    // -1 Because no connecting paths for the first map
    createPath(map, PATH, -1, -1, -1, -1); 
    addTerrain(map, TALL_GRASS, 2);
    addTerrain(map, WATER, 1);
    addOtherFeatures(map);
    placeBuilding(map, POKEMON_CENTER);
    ...
    while (1) { 
        ...
        if (userInput == 'q') {
            ...
            // Save the current map before moving
            saveMap(maps, map, currentX, currentY);
            ...
            // Load the map for current position
            ...
            if (newRow >= -200 && newRow < 200 && newCol >= -200 && newCol < 200) {
                saveMap(maps, map, currentX, currentY);
                // Adds two hundred because the center is technically (200, 200)
                currentX = newCol +200; 
                currentY = newRow +200;
                //Will load or generate new map
                loadMap(maps, map, currentX, currentY); 
                printMap(map); 
            } else {
                printf("Invalid coordinates. Please enter values between -200 and 199 for row and between -200 and 199 for column.\n");
            }
        } else {
            printf("Invalid input. Please enter 'n', 'e', 's', 'w', 'f' to move or 'q' to quit.\n");
        }
    }

    // Memory cleanup: free all allocated maps
    for (int i = 0; i < MAX_MAPS; i++) {
        for (int j = 0; j < MAX_MAPS; j++) {
            if (maps[i][j]) {
                free(maps[i][j]);
